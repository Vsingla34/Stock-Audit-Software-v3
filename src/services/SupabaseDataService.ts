...
class SupabaseDataService {
  ...
  private inventoryItemToDb(item: Partial<InventoryItem>): any {
    ...
    if (item.auditorEntries && Array.isArray(item.auditorEntries) && item.auditorEntries.length > 0) {
      try {
  ...
  public async validateSKUsExist(skus: string[]): Promise<{ valid: boolean; missingSKUs: string[] }> {
    if (skus.length === 0) {
...
public async getItemMaster(companyId?: string): Promise<InventoryItem[]> {
  ...
  public async setClosingStock(items: Partial<InventoryItem>[]): Promise<void> {
    ...
    if (!hasMaster) {
      throw new Error("VALIDATION_ERROR: Item master must be uploaded before closing stock. Please contact admin to upload item master first.");
    }
  ...
  public async addLocation(location: Omit<Location, 'id'>): Promise<Location> {
    try {
  ...
  public async clearInventoryData(): Promise<void> {
    try {
      ...
      await supabase.from('questionnaire_answers').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    } catch (error) {
